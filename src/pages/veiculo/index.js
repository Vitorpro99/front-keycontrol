...
export default function VeiculoPage() {
    ...
    const [estiloPlaca, setEstiloPlaca] = useState(styles.placaNeutra);
    ...
    useEffect(() => {
        const valorLimpo = placa.replace(/[^a-zA-Z0-9]/g, '').toUpperCase(); // Remove traços e espaços

        // Regex Padrão Antigo (3 letras, 4 números)
        const isAntiga = /^[A-Z]{3}[0-9]{4}$/.test(valorLimpo);
        
        // Regex Mercosul (3 letras, 1 número, 1 letra, 2 números)
        const isMercosul = /^[A-Z]{3}[0-9][A-Z][0-9]{2}$/.test(valorLimpo);

        if (isMercosul) {
            setEstiloPlaca(styles.placaMercosul);
        } else if (isAntiga) {
            setEstiloPlaca(styles.placaCinza);
        } else {
            setEstiloPlaca(styles.placaNeutra);
        }
    }, [placa]);
            ...
            <div  className={styles.formDiv}>
                <form onSubmit={handleSubmit} className ={styles.form}>
                    ...
                    <div style={{display: 'flex', flexDirection: 'column', alignItems: 'center'}}>
                        ...
                        <input 
                            // AQUI APLICAMOS AS DUAS CLASSES
                            className={`${styles.inputPlaca} ${estiloPlaca}`} 
                            name="placa" 
                            type="text" 
                            placeholder="ABC1234" 
                            value={placa} 
                            onChange={(e) => setPlaca(e.target.value.toUpperCase())} 
                            maxLength={8} // Limita tamanho
                            required 
                        />
    
                        <small style={{color: '#666', marginTop: '5px', fontSize: '0.8rem'}}>
                            {estiloPlaca === styles.placaMercosul ? "Padrão Mercosul Detetado" : 
                             estiloPlaca === styles.placaCinza ? "Padrão Antigo Detetado" : "Digite a placa..."}
                        </small>
                    </div>
